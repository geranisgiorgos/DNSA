...
typedef unsigned int NODE;		/* node number */
typedef  long ARC;		        /* arc number */
typedef int CAPACITY;			/* arc capacity */
typedef int COST;			    /* arc cost */
typedef unsigned long INDEX;	/* index element */
typedef int INDEX_LIST;			/* index list handle */
//float w1[NODES],w2[NODES];
float *w1,*w2;

//int thesi_w1[NODES1], thesi_w2[NODES1];
int *thesi_w1, *thesi_w2;

// T contains the nodes that have been chosen, If T[i]==1 then i has been chosen
//int T[NODES1];
int *T;
...
/*
#define MAXNODES 17000	    		{COM maximum problem sizes COM}
#define MAXARCS  150000
*/
#define MAXNODES 150000	    		/* maximum problem sizes */
#define MAXARCS 1500000
