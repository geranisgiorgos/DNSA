...
//Initialization of arrays (using malloc) used in netgen
void init_gen(){
    w1=(float *)malloc((NODES)*sizeof(float));
    w2=(float *)malloc((NODES)*sizeof(float));
    thesi_w1=(int *)malloc((NODES1)*sizeof(int));
    thesi_w2=(int *)malloc((NODES1)*sizeof(int));
    T=(int *)malloc((NODES1)*sizeof(int));
}


/*** Fortran callable interface routine */

void netgen_(long* seed,long parms[PROBLEM_PARMS],long* generated_nodes,long* generated_arcs){
...
ARC netgen(long seed, long parms []){
  ...
  int sort_count;
    ...
    //Initialization of the arrays used
    init_gen();
...
/* changing to handle overflows with large n; Mar 18 -- jc */
      sinks_per_source = (NODE)(((double) 2*sort_count*SINKS) / ((double) NODES-SOURCES-SINKS));
...
PRIVATE void create_supply(NODE sources, CAPACITY supply){
...
PRIVATE void create_assignment(long parms[]){
...
PRIVATE void sort_skeleton(int sort_count){
  int m,i,j,k;
  int temp;
...
PRIVATE void pick_head(long parms[],INDEX_LIST handle,NODE desired_tail){
...
PRIVATE void error_exit(long rc){
  switch (rc) {
    case BAD_SEED:
